...
start = dt.datetime.strptime(order.ix[0,'Date'],"%Y-%m-%d %H:%M:%S")
end = dt.datetime.strptime(order.ix[len(order.iloc[:,1])-1,'Date'],"%Y-%m-%d %H:%M:%S")
